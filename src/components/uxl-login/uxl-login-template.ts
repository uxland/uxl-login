...
export const template = (props: UxlLogin) => html`${uxlLoginStylesNative}
  <div class="login__container" part="login__container">
    ${props.mainImage
      ? html`
        ...
        `
      : nothing}
    ${props.isTitleVisible ? html` <div class="title" part="title">${props.titleText}</div> ` : nothing}
    ${props.displayName || props.userImage
      ? html`
          <div class="user__info" part="user__info">
            ${props.userImage ? html`<img class="user__info-image" src="${props.userImage}" part="user__info-image" />` : nothing}
            ${props.displayName
              ? html` <div class="user__info-welcome-message" part="user__info-welcome-message">${props.welcomeMessage} ${props.displayName}</div> `
              : nothing}
          </div>
        `
      : nothing}
    <form class="login__form" part="login__form">
      ...
      <div class="password-container" part="password-container">
        <paper-input
          ...
          ?always-float-label="${props.showAlwaysFloatLabel}"
        ></paper-input>
      </div>
      ${props.errorMessage ? html` <div class="error__message" part="error__message">${props.errorMessage}</div> ` : nothing}
    ...
    ${props.isFooterVisible ? html` <footer class="footer" part="footer">${props.footerText}</footer> ` : nothing}
  </div> `;
